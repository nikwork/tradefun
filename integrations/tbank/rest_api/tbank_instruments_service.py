...
class Quotation:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "Quotation":
        """Create from dictionary"""
        return cls(units=int(data.get("units", 0)), nano=int(data.get("nano", 0)))
...
class BaseInstrumentsClient:
    ...
    PRODUCTION_URL = "https://invest-public-api.tbank.ru/rest"
...
class InstrumentsService(BaseInstrumentsClient):
    ...
    Usage:
        ...
        client = InstrumentsService(token="your_api_token", sandbox=True)
    ...
    Or with context manager:
        with InstrumentsService(token="your_token", sandbox=True) as client:
            bonds = client.bonds()
    """
    ...
    def __init__(
        ...
        Args:
            ...
            sandbox: Use sandbox environment (default: False)
                    - False: Production environment (real trading)
                    - True: Sandbox environment (test trading)
            verify_ssl: Verify SSL certificates (default: True)
                    - True: Verify SSL (recommended for production)
                    - False: Skip SSL verification (for sandbox with SSL issues)

        Example:
            # Production
            client = InstrumentsService(token="your_token")

            # Sandbox (with SSL verification disabled if needed)
            client = InstrumentsService(token="your_token", sandbox=True, verify_ssl=False)
        """
        self.token = token
    ...
    def _request(self, endpoint: str, payload: Dict[str, Any] = None) -> Dict[str, Any]:
        ...
        try:
            response = self.session.post(
                url, json=payload or {}, timeout=self.timeout, verify=self.verify_ssl
            )
    ...
    def get_futures_margin(self, instrument_id: str) -> Dict[str, Any]:
        """
    ...
    def get_consensus_forecasts(
        self, paging: Optional[Dict[str, int]] = None
    ) -> Dict[str, Any]:
        """
    ...
    def get_forecast_by(self, instrument_id: str) -> Dict[str, Any]:
        """
    ...
    def create_favorite_group(
        self, name: str, instruments: Optional[List[Dict[str, str]]] = None
    ) -> Dict[str, Any]:
        """
    ...
    def edit_favorites(
        self, instruments: List[Dict[str, Any]], action_type: str
    ) -> Dict[str, Any]:
        """
        ...
        """
        payload = {"instruments": instruments, "actionType": action_type}
...
class AsyncInstrumentsService(BaseInstrumentsClient):
    ...
    Usage:
        ...
        # Sandbox
        async with AsyncInstrumentsService(token="your_token", sandbox=True) as client:
            bonds = await client.bonds()
    """
